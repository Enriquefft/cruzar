# packages/career-ops — operator rules

This package is absorbed critical-path tooling from an external personal pipeline per [ADR-03](../../product/cruzar/adr/03-career-ops-absorb-scope.md). It is **not** one-person tooling; it is multi-tenant infrastructure driven by `scripts/run-cohort.ts` in `apps/web`-adjacent code, on behalf of any student in the cohort.
