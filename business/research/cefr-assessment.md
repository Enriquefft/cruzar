...
**Reliability:** `Estimate:` GPT-4-class models achieve ~85-90% adjacent-level agreement with trained human raters. `Estimate:` human inter-rater agreement itself is ~80-90%. TODO sourcing: needs citation to academic study or vendor benchmark.
...
`View:` browser audio capture → Deepgram STT → Claude rubric scoring → CEFR level + sub-scores. Five questions. `Estimate:` accurate within one level for 85%+ of test-takers.

The MSP build decision belongs in [product/cruzar/](../../product/cruzar/), not here.
